...
本发行包提供 Markdown 图片命名/搬运的一体化工具集，包含批量 GUI、命令行核心脚本以及可选的图片本地化辅助脚本。V1 对 GUI 进行了完整修复，并新增了“翻译 / 归纳”上下文助手；v1.1.0 在此基础上继续增强批量处理策略与可配置性。
...
- 新增「SCI」命名策略选项：批量 GUI 的“命名策略”中支持选择 SCI，并对其处理流程与配置项进行了梳理与优化。
- 模板占位/待办支持：批量 GUI 增强了模板占位能力，并引入待办列表以便分段处理与复核。
- 配置与安全：新增忽略本地 GUI 配置档的规则，避免意外提交含敏感信息的文件；保留你在本机的个人 API/模型配置。
- 其他：依赖与文档微调，移除无关的编辑器/备份产物以保持仓库整洁。
...
此 README 所在目录可视为发行包参考结构；若需分享给其他用户，可直接压缩整个 `release_bundle/` 目录。也可前往 GitHub Releases 页面下载对应版本的压缩包（若提供）。
